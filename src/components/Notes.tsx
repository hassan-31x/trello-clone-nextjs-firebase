...
const Note = (props: NoteProps) => {
  ...
  const handleContentChangeNote = (e: React.ChangeEvent<HTMLInputElement>, objectId: string, noteId: string) => {
  ...
  const editContent = async (changeContainer: INotesList, noteId: string) => {
    if (props.userId) {
      const noteRef = doc(db, "users", props.userId, "noteContainer", changeContainer.id, "notes", noteId);
      if (changeContainer) {
  ...
  const deleteElement = async (contanerId: string, noteId: string) => {
    if (props.userId) {
      try {
        ...
        const noteContainerRef = doc(userDocRef, "noteContainer", contanerId);
        const noteRef = doc(collection(noteContainerRef, "notes"), noteId);
  ...
  return (
    <Draggable key={props.note.id} draggableId={props.note.id} index={props.index}>
      {(provided, snapshot) => (
        <div className={`note ${snapshot.isDragging ? "bg-yellow-300 skew-y-6" : "bg-white skew-y-0"} select-none rounded-xl my-2 flex item-center px-3 py-2 overflow-auto`}
          onMouseEnter={() => setHover(`${props.note.id}`)} onMouseLeave={() => setHover("")}
          ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps} style={{ ...provided.draggableProps.style }}>

          <div className="w-full flex justify-between">
            {props.note.content.substring(0, 38) === "https://firebasestorage.googleapis.com" ? <img src={props.note.content} className="h-20" /> : (
              
              <div>
                {props.note.isEditable ?
                  <input type="text" value={props.note.content} onChange={(e) => handleContentChangeNote(e, props.container.id, props.note.id)} onBlur={() =>toggleEditModeNote(props.container, props.note.id)} className="bg-[white] pl-[0.58rem] py-1 border-none" autoFocus />
                : <span className="pl-3">{props.note.content}</span>
                }
              </div>

            )}
            <div className={`buttons flex gap-2 ${hover.includes(props.note.id) ? "block" : "hidden"} ${props.note.isEditable ? "hidden" : "block"}`}>
              <button type="button" onClick={() => toggleEditModeNote(props.container, props.note.id)}><BiPencil /></button>
              <button type="button" onClick={() => { deleteElement(props.container.id, props.note.id)}}><MdDeleteOutline /></button>
            </div>
          </div>
